...
Surge$text = gsub("\n", " ", Surge$text)
SurgeRefined <- Surge %>% transmute(text = trimws(gsub(" ?(f|ht)tp(s?)://(.*)[.][a-z]+", "", gsub("[^A-Za-z0-9 ]","",text))), toxic = (Is.this.text.toxic. == "Toxic"))
...
PaperRefined <- PaperRefined %>% mutate(text = gsub("@\\w+ *", "", gsub("RT", "", text)))
...
KaggleRefined <- Kaggle %>%  transmute(text = trimws(gsub(" ?(f|ht)tp(s?)://(.*)[.][a-z]+", "", gsub("[^A-Za-z0-9 ]","",comment_text))), toxic = (toxic != 0 | severe_toxic != 0 | obscene != 0 | threat != 0 | insult != 0 | identity_hate != 0))
...
write.csv(Final, "/Users/mateuszkazimierczak/Downloads/FinalDataset.csv", row.names=FALSE)
